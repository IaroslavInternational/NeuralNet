...
SoundSystem& SoundSystem::Get()
{
 ...
 void SoundSystem::SetMasterVolume( float vol )
 {
	 ...
	 {
		throw ( hr,L"Setting master volume" );
	 }
 }
	...
	format( std::make_unique<WAVEFORMATEX>() )
{
	...
	{
		throw ( hr,L"Creating factory for XAudio2 object" );
	}
	...
	xaBuffer( std::make_unique<XAUDIO2_BUFFER>() )
{
	...
	{
		throw ( hr,L"Creating source voice for channel" );
	}
}
...
void SoundSystem::Channel::PlaySoundBuffer( Sound& s,float freqMod,float vol )
{
	...
	{
		throw ( hr,L"Starting playback - submitting source buffer" );
	}
	if( FAILED( hr = pSource->SetFrequencyRatio( freqMod ) ) )
	{
		throw ( hr,L"Starting playback - setting frequency" );
	}
	if( FAILED( hr = pSource->SetVolume( vol ) ) )
	{
		throw ( hr,L"Starting playback - setting volume" );
	}
	if( FAILED( hr = pSource->Start() ) )
	{
		throw ( hr,L"Starting playback - starting" );
	}
}
	...
	float loopStartSeconds,float loopEndSeconds )
{
	...
	{
		{
...

			{
				...
				{
					throw ( fileName,L"Bad fourcc code" );
				}
			}
			...
			if( fileSize <= 44u )
			{
				throw ( fileName,L"file too small" );
			}
		...
		{
			throw ( fileName,L"format not WAVE" );
		}
		...
		if( !bFilledFormat )
		{
			throw ( fileName,L"fmt chunk not found" );
		}
		...
		// compare format with sound system format
		{
			...
			{
				throw ( fileName,L"bad wave format (nChannels)" );
			}
			else if( format.wBitsPerSample != sysFormat.wBitsPerSample )
			{
				throw ( fileName,L"bad wave format (wBitsPerSample)" );
			}
			else if( format.nSamplesPerSec != sysFormat.nSamplesPerSec )
			{
				throw ( fileName,L"bad wave format (nSamplesPerSec)" );
			}
			else if( format.wFormatTag != sysFormat.wFormatTag )
			{
				throw ( fileName,L"bad wave format (wFormatTag)" );
			}
			else if( format.nBlockAlign != sysFormat.nBlockAlign )
			{
				throw ( fileName,L"bad wave format (nBlockAlign)" );
			}
			else if( format.nAvgBytesPerSec != sysFormat.nAvgBytesPerSec )
			{
				throw ( fileName,L"bad wave format (nAvgBytesPerSec)" );
			}
		}
		...
		if( !bFilledData )
		{
			throw ( fileName,L"data chunk not found" );
		}
		...
		case LoopType::AutoEmbeddedCuePoints:
			{
				...
				if( !bFilledCue )
				{
					throw ( fileName,L"loop cue chunk not found" );
				}
			}
			break;
	...
	catch( const std::exception& e )
	{
		...
		throw ( fileName,std::wstring( what.begin(),what.end() ) );
	}
}
